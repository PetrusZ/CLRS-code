 ...
 *       Filename:  os_tree.c
 *
 *    Description:  顺序统计树(order-statistic tree)
 *
 *        Version:  1.0
 *        Created:  2013年11月20日 20时39分51秒
 *       Compiler:  gcc
 *
...
typedef struct rb_node {
    ...
    int     key;
    int     color;
    int     size;       //x.size = x.left.size + x.right.size + 1
}RB_NODE;
...
void os_fixwalk(RB_TREE *, RB_NODE *);
int os_rank(RB_TREE *, RB_NODE *);
